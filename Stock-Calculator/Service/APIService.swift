...
struct APIService {
    ...
    func fetchMonthlyAdjustedPublisher(keywords: String) -> AnyPublisher<MonthlyAdjusted, Error> {
        ...
        case .success(let url):
            return URLSession.shared.dataTaskPublisher(for: url)
                .map({ $0.data })
                .decode(type: MonthlyAdjusted.self, decoder: JSONDecoder())
                .receive(on: RunLoop.main)
                .eraseToAnyPublisher()
        case .failure(let error):
            return Fail(error: error).eraseToAnyPublisher()
    ...
    func parseQuery(text: String) -> Result<String, Error> {
        if let query = text.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) {
            ...
            return .failure(ServiceError.encoding)
        }
    ...
    func parseURL(urlString: String)-> Result<URL, Error> {
        if let url = URL(string: urlString) {
            ...
            return .failure(ServiceError.badRequest)
        }
    }
